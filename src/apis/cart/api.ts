...
export const addCartApi = async (data: {
  ...
  const response = await customHttpInstance('http://13.114.2.60:8082/api/v1').post('/cart/add-to-cart', data)
...
export const deleteCartItemApi = async (productId: string, size: string) => {
  const response = await customHttpInstance('http://13.114.2.60:8082/api/v1').delete(`/cart/delete-cart`, {
    data: { productId, size }
  })
  return response
}
